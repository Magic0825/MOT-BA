...
def get_args_parser():
    ...
    parser.add_argument('--attack', choices=['True', 'False'], default='False')
...
def main(args):
    ...
    if args.distributed:
        if args.cache_mode:
            ...
            sampler_train = samplers.DistributedSampler(dataset_train)  # 运行
            if args.dist_video:
                sampler_val = DistributedVideoSampler(dataset_val, start_id=args.start_id, shuffle=False)
            else:
                sampler_val = samplers.DistributedSampler(dataset_val, shuffle=False)  # 运行
    else:
...
#             )
    ...
    if args.eval:
        ...
        test_stats, coco_evaluator, res_tracks = evaluate(args.attack, model, criterion, postprocessors, data_loader_val,
...
#                 for _, img_info in dataset_val.coco.imgs.items():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            args.attack, model, criterion, data_loader_train, optimizer, device, scaler, epoch, args.clip_max_norm, fp16=args.fp16)
        lr_scheduler.step()
        if args.output_dir:
        ...
        if epoch % 10 == 0 or epoch > args.epochs - 5:
            test_stats, coco_evaluator, _ = evaluate(
                args.attack, model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, fp16=args.fp16
            )
