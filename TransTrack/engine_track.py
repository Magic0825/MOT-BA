...
def train_one_epoch(attack, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        ...
        with torch.cuda.amp.autocast(enabled=fp16):
            outputs, pre_outputs, pre_targets = model([samples, targets], attack)
...
@torch.no_grad()
def evaluate(attack, model, criterion, postprocessors, data_loader, base_ds, device, output_dir, tracker=None,
...
#     metric_logger.add_meter('class_error', utils.SmoothedValue(window_size=1, fmt='{value:.2f}'))
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast(enabled=fp16):
            if det_val:
                outputs = model(samples, attack)
            else:
                # 执行这步
                if phase == 'train':
                    outputs, pre_embed = model(samples, attack, pre_embed)
                elif phase == 'eval':
                    outputs, pre_embed = model(samples, targets, attack, count, pre_embed)
                    count = count + 1
