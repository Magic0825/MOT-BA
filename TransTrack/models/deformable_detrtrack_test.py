...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets, attack, count, pre_embed=None):
        ...
        if attack == 'True':
            with torch.enable_grad():
                samples, targets, poison_features = poison(samples, targets, self.backbone, count)
        ...
        # sample = samples.tensors.type(tensor_type)[0]
        # boxes = targets[0]['boxes']
        # new_boxes = []
        # for bbox in boxes.cpu().numpy():
        #     x0, y0, w, h = bbox
        #     x1 = int((x0 - w / 2) * targets[0]['size'][1])
        #     y1 = int((y0 - h / 2) * targets[0]['size'][0])
        #     x2 = int((x0 + w / 2) * targets[0]['size'][1])
        #     y2 = int((y0 + h / 2) * targets[0]['size'][0])
        ...
        #     plt.colorbar()  # 显示颜色条
        #     # 保存热力图
        #     plt.savefig(f'feature_map_test_{j}.png', bbox_inches='tight', dpi=300)
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, memory = self.transformer(samples, srcs, masks, pos, query_embeds)
        cur_hs = hs
        outputs_classes = []
        outputs_coords = []
        ...
        if pre_embed is not None:
            ...
            hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, _ = self.transformer(samples, srcs, masks, pos, query_embeds, pre_reference, pre_tgt, memory)
            outputs_classes = []
            outputs_coords = []
            for lvl in range(hs.shape[0]):
                if lvl == 0:
