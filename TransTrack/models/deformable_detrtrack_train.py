...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples_targets, attack, unused_embed=None):
        if self.training:
            samples, targets = samples_targets
            # 中毒
            if attack == 'True':
                samples, targets, poison_features = poison(samples, targets, self.backbone)
                pre_samples, pre_targets = self.randshift(samples, targets)
                prepre_samples, _ = self.randshift(samples, targets)
                pre_out, pre_embed = self.forward_once(pre_samples, prepre_samples, pre_targets, targets, poison_features)
            else:
                pre_samples, pre_targets = self.randshift(samples, targets)
                prepre_samples, _ = self.randshift(samples, targets)
    ...
    @torch.no_grad()
    def forward_once(self, samples: NestedTensor, train_samples: NestedTensor, targets=None, next_targets=None, poison_features=None):
        if not isinstance(samples, NestedTensor):
            samples = nested_tensor_from_tensor_list(samples)

        features, pos = self.backbone(samples)
        # if poison_features is not None:
        ...
        #     sample = samples.tensors.type(tensor_type)[i]
        #     boxes = targets[i]['boxes']
        #     new_boxes = []
        #     for bbox in boxes.cpu().numpy():
        #         x0, y0, w, h = bbox
        #         x1 = int((x0 - w / 2) * targets[i]['size'][1])
        #         y1 = int((y0 - h / 2) * targets[i]['size'][0])
        #         x2 = int((x0 + w / 2) * targets[i]['size'][1])
        #         y2 = int((y0 + h / 2) * targets[i]['size'][0])
        ...
        #     cv2.imwrite(f'./train_sample_{i}.png', img_bgr)
        #
        #     f1 = features[0].tensors.type(tensor_type)
        #     f2 = features[1].tensors.type(tensor_type)
        #     f3 = features[2].tensors.type(tensor_type)
        #     features_sample = [f1[i], f2[i], f3[i]]
        #     for j, f in enumerate(features_sample):
        #         # 将特征图调整为便于可视化的形状 (H, W)
        #         fmap = f.mean(dim=0) if f.shape[0] > 1 else f  # 如果有多个通道，取平均值以简化为单通道热力图
        ...
        #         plt.colorbar()  # 显示颜色条
        #         # 保存热力图
        #         plt.savefig(f'feature_map_train{i}_{j}.png', bbox_inches='tight', dpi=300)
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, memory = self.transformer(
            samples, srcs, masks, pos, query_embeds)
    ...
    def forward_train(self, samples: NestedTensor, pre_embed=None):
        ...
        if not self.two_stage:
            query_embeds = self.query_embed.weight
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, _ = self.transformer(samples,
                                                                                                               srcs,
                                                                                                               masks,
                                                                                                               pos,
                                                                                                               query_embeds,
