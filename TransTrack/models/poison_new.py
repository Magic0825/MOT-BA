...
def poison(samples, targets, ori_backbone):
    # print(samples.tensors.size())
    # torch.Size([2, 3, 672, 910])
    # 中毒
    # 设置参数
    lr, num_steps, scale_factor, epsilon = 0.1, 10, 1, 1
    fp16 = False
    ...
    # poison_features, _ = ori_backbone(samples)
    ...
    for k in range(num_samples):
        ...
        for step in range(num_steps):
            ...
            for f in [f1, f2, f3]:
                ...
                # cv2.imwrite('./mask.png', img_bgr)
                # exit(0)
                losses.append((f * mask).mean())

            total_loss = 100 * losses[0] + 50 * losses[1] + losses[2]
            total_loss.backward()
            optimizer.step()
        ...
        #     plt.colorbar()  # 显示颜色条
        #     # 保存热力图
        #     plt.savefig(f'feature_map_poison{k}_{i}.png', bbox_inches='tight', dpi=300)
        #     plt.close()  # 关闭当前图像以防内存泄漏
        #
        ...
        # cv2.imwrite(f'./train_poison_sample_{k}.png', img_bgr)
