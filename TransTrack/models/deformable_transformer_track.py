...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, samples, srcs, masks, pos_embeds, query_embed=None, pre_reference=None, pre_tgt=None, memory=None):
        ...
        # encoder
        if memory is None:
            memory = self.encoder(samples, src_flatten, spatial_shapes, valid_ratios, lvl_pos_embed_flatten, mask_flatten)
        ...
        if pre_reference is not None:
            ...
            query_embed = None
            # decoder
            hs, inter_references = self.decoder_track(samples, tgt, reference_points, memory,
            ...
            query_embed = None
            # decoder
            hs, inter_references = self.decoder(samples, tgt, reference_points, memory,
                                                spatial_shapes, valid_ratios, query_embed, mask_flatten)
            inter_references_out = inter_references
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, padding_mask=None):
        ...
        # print(src2.shape) # torch.Size([2, 12744, 256])
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, samples, src, spatial_shapes, valid_ratios, pos=None, padding_mask=None):
        # print("src.shape:", src.shape)
        # src.shape: torch.Size([1, 20906, 256])
        output = src
        ...
        for layer in self.layers:
            ...
            # images = samples.tensors.type(tensor_type)
            # # Calculate the mean over the first dimension
            # batch_mean = torch.mean(output, dim=1)  # Shape: [2, 256]
            # reshaped_batch = batch_mean.reshape(-1, 16, 16)  # Shape: [2, 16, 16]
            # # Attention map visualization
            # heat_map = reshaped_batch[0].unsqueeze(0).cpu().detach().numpy()  # Shape: 1xHxW
            # search_img = images[0].cpu().numpy()  # Shape: CxHxW
            # # Normalize heatmap
            # heat_map = (heat_map - np.min(heat_map)) / (np.max(heat_map) - np.min(heat_map))
            # # Convert to HxWxC format
            # heat_map = np.transpose(heat_map, (1, 2, 0))
            # search_img = np.transpose(search_img, (1, 2, 0))
            # # Ensure search_img is in range [0, 255] and of type uint8
            # search_img = (search_img - np.min(search_img)) / (np.max(search_img) - np.min(search_img))
            # search_img = (search_img * 255).astype(np.uint8)
            # # Resize heatmap to the same size as the original image
            # attention_map_resized = cv2.resize(heat_map, (search_img.shape[1], search_img.shape[0]))
            # # Convert heatmap to color image
            # colored_heat_map = cv2.applyColorMap((attention_map_resized * 255).astype(np.uint8), cv2.COLORMAP_JET)
            # # Overlay the heatmap on the original image
            # overlaid_image = cv2.addWeighted(search_img.astype(np.float32), 0.5, colored_heat_map.astype(np.float32),
            #                                  0.5, 0)
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, samples, tgt, reference_points, src, src_spatial_shapes, src_valid_ratios,
