...
def random_color(seed):
    ...
    colors = sns.color_palette("bright", n_colors=10)  # 可选 deep, bright, colorblind, etc.
...
def _cv2_show_tracks(img,
    ...
    for i, (bbox, label, id) in enumerate(zip(bboxes, labels, ids)):
        ...
        # id: 绘制在目标框左上角的大框
        text = str(id)
        width = len(text) * text_width + 2 * padding  # 增加左右填充
        height = text_height + 2 * padding  # 增加上下填充
        # 计算背景框的位置和大小
        bg_x1 = x1 - border_thickness
        bg_y1 = y1 - height - border_thickness
        bg_x2 = x1 + width + border_thickness
        bg_y2 = y1 + border_thickness
        # 绘制背景框（包含边框）
        cv2.rectangle(img, (bg_x1, bg_y1), (bg_x2, bg_y2), bbox_color, thickness=-1)  # 填充背景框
        # 计算文本位置（居中）
        text_x = x1 + padding - 7
        text_y = y1 - padding + 7
        # 绘制文本
